...
const SearchResults = () => {
  ...
  const filteredPersonnel = personnel.filter(person =>
    person.pets.some(pet => selectedPets.includes(pet)) && person.location === userLocation
  );
  ...
  return (
    <div className="p-4">
      ...
      {filteredPersonnel.length > 0 ? (
        <p className="mb-2">{filteredPersonnel.length} search results found in <strong>{userLocation}</strong> for the following pets:</p>
      ) : (
        <p className="mb-2">No search results found in <strong>{userLocation}</strong> for the selected pets.</p>
      )}


<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
  {filteredPersonnel.map(person => (
    <div key={person.id} className="bg-white rounded-lg overflow-hidden shadow-md transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-105">
      <div className="bg-gradient-to-r from-green-400 to-pink-500 text-center py-2">
      ...
      <div className="p-4">
        ...
        <div className="flex items-center mt-2">
          ...
          <span className="text-gray-700 ml-1">{person.location}</span>
        </div>
        <div className="flex items-center mt-4">
          <FaPhone className="text-gray-500" />
          <button className="text-purple-500 ml-2 font-semibold hover:underline">Contact Us</button>
        </div>
      </div>
    </div>
  ))}
      ...
      {additionalPersonnel.length > 0 && (
        <>
          <p className="mt-6 text-2xl font-bold">Explore more pet sitters in <strong>{userLocation}</strong>:</p>
